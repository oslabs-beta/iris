...
const add_failedpartitionscount_value = async (lastTimeStamp) => {
  try {
    const res = await fetch(`${BASE_PATH}${PROM_QUERY}kafka_server_replica_fetcher_manager_failedpartitionscount_value[5m]`);
    ...
    console.log('ERROR: add_failedpartitionscount_value', err);
  }
};

module.exports = add_failedpartitionscount_value